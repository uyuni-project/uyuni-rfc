...
Some examples to the feedback messages which can be reported by an entity are as follows:
 ...
 - [INFO] "Since no module filters are added to the project, 'centos8-appstream' will be cloned as a modular repository" [reported from the centos-appstream project source]
 - [WARNING] "No modular repository sources are added to the project. Module filters will be ignored" [reported from the module filters, duplicates removed]
 - [ERROR] "'postgresql:9.6' conflicts with 'postgresql:10'" [reported from postgresql:9.6 filter]
