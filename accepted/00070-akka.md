...
Some shortcuts have been taken during HackWeek and will not be acceptable for proper implementation, particularly:
 ...
 - the `DeferringActor` implementation is at the moment not completely written in stateless style
...
Akka has an optional module called Clustering that allows to transparently move computation across processes, even on different machines in a network. It is thus possible to instantiate Actors "remotely", and Akka takes care of serializing, delivering and deserializing Messages to them transparently.
